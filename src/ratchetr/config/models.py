...
"""Configuration models and validation for ratchetr.

This module defines the data models for ratchetr configuration, including both
...
from ratchetr.exceptions import RatchetrValidationError
...
class UnsupportedConfigVersionError(ConfigValidationError):
    ...
    def __init__(self, provided: int, expected: int) -> None:
        ...
        Args:
            ...
            expected: The config_version value expected by this version of ratchetr.
        """
...
class DirectoryOverrideValidationError(ConfigValidationError):
    ...
    def __init__(self, path: Path, error: Exception) -> None:
        ...
        self.path = path
        self.error = error
        super().__init__(f"Invalid ratchetr directory override in {path}: {error}")
...
class InvalidConfigFileError(ConfigValidationError):
    ...
    def __init__(self, path: Path, error: Exception) -> None:
        ...
        self.path = path
        self.error = error
        super().__init__(f"Invalid ratchetr configuration in {path}: {error}")
...
class AuditConfig:  # pylint: disable=too-many-instance-attributes
    ...
    Attributes:
        ...
        default_include: List of specific file paths to audit. If provided, only these
            files are checked.
...
class PathOverrideModel(BaseModel):
    ...
    TOML files (ratchetr.dir.toml, .ratchetrdir.toml). After validation, it is converted
...
class AuditConfigModel(BaseModel):
    ...
    Attributes:
        manifest_path: Optional path to save the audit manifest file.
        default_include: List of specific file paths to audit.
        max_depth: Maximum directory depth for recursive file discovery.
        max_files: Maximum number of files to include in the audit.
        max_bytes: Maximum total size in bytes of files to include in the audit.
        skip_current: Whether to skip files in the current directory.
    ...
    @field_validator("default_include", "runners", mode="before")
    @classmethod
...
class ConfigModel(BaseModel):
    """Pydantic model for validating the top-level ratchetr configuration from TOML.
