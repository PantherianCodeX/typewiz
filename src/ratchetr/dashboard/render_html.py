...
def _render_engines_tab(context: _DashboardContext) -> list[str]:
    ...
    if context.engines:
        for key, data in context.engines.items():
            ...
            default_paths = _coerce_str_list(options.get("include"))
            ...
            include_html = _format_code_list(default_paths, h) if default_paths else "—"
...
def _format_override_html(entry: OverrideEntry, escape_fn: Callable[[object], str]) -> str:
    path, profile, plugin_args, default_paths, exclude_paths = get_override_components(entry)
    details: list[str] = []
    if plugin_args:
        details.append(f"plugin args={_format_code_list(plugin_args, escape_fn)}")
    if default_paths:
        details.append(f"include={_format_code_list([str(p) for p in default_paths], escape_fn)}")
    if exclude_paths:
