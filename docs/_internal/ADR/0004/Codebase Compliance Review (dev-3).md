...
1. **Unit test directories do not consistently mirror product domains:**
   ...
   * **Correction:** Relocate tests into domain-aligned paths (e.g., `tests/unit/core/`, `tests/unit/_infra/`, `tests/unit/paths/` or `tests/unit/runtime/` depending on what they’re testing). If you want meta/guardrail tests, introduce a narrowly-scoped `tests/unit/architecture/` and keep it minimal.
...
4. **Root package contains substantive implementation (`ratchetr/json.py`):**
   ...
   * **Correction (strict façade):**
     ...
     * Move implementation to `_infra/utils/json.py` (or a more specific private namespace under `_infra/` if preferred).
...
6. **Root façade imports must not depend on non-façade root implementations:**
   ...
   * **Correction:** After moving JSON implementation behind `_infra/`, ensure root façades import from `_infra/*` or feature packages directly, not from other root implementation modules.
