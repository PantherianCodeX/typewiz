...
**Purpose:** Govern the ADR + supplementary-doc rewrite/migration so we (1) preserve the strongest architecture from the draft-2 ADR set, and (2) apply the deltas from **ADR Rewrite Plan v18** without duplication, drift, or scope creep.  
...
**Primary links:** `ADR Rewrite Plan v18.md` (execution plan); Draft-2 ADR set (preservation inputs); registries in `docs/_internal/policy/execution-contract/`.
...
- Controlled overlay of ADR Rewrite Plan v18 deltas on top of preserved architecture.
...
- Replacing ADR Rewrite Plan v18 (this policy *governs* how the plan is applied; it is not a rewrite-plan).
...
2. `ADR Rewrite Plan v18.md` (the execution plan and resolved deltas for the rewrite).
...
## 5. Plan overlay protocol (v18 deltas)

### 5.1 Overlay mandate
All incoming deltas from ADR Rewrite Plan v18 must be applied *without* duplicating concepts or destabilizing preserved architecture.
...
(See `ADR Rewrite Plan v18.md` for the exact rule and examples.)
...
- `docs/_internal/policy/execution-contract/templates/draft-log-template.md`
...
Canonical location:
- `docs/_internal/policy/execution-contract/`
...
- Applying plan-v18 deltas as recorded in `plan_overlay_register.md`.

### 8.2 Prohibited behaviors
- Inventing missing semantics to “fill gaps”.
- Adding new feature requirements outside plan-v18.
