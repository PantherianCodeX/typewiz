...
The `ratchetr` package collects typing diagnostics from Pyright, mypy, and other plugins and
...
- Environment overrides: set `RATCHETR_LOG_FORMAT` / `RATCHETR_LOG_LEVEL` to enforce defaults when the CLI flags are not provided.
- Programmatic logging: `ratchetr.logging.configure_logging()` and `ratchetr.logging.structured_extra()` ensure every log record shares the same schema.
...
Place a `ratchetr.dir.toml` (or `.ratchetrdir.toml`) within a subdirectory to scope additional configuration to that tree:
...
Each ratchet is keyed to the tool, mode, and fully merged engine options (profile, plugin arguments, include/exclude paths, overrides, category mappings). If any of those change—for example, enabling a Ruff runner—`ratchetr ratchet check` will flag a signature mismatch so you deliberately rebuild the baseline instead of silently accepting stale budgets.

## Engines & plugins

ratchetr loads engines from the built-in registry (`pyright`, `mypy`) and from any entry points exposed under
the `ratchetr.engines` group. Provide a custom engine by implementing the
`ratchetr.engines.base.BaseEngine` protocol and declaring it in your package's `pyproject.toml`:
...
class MyRunner(BaseEngine):
    ...
    def run(self, context: EngineContext, paths: Sequence[str]) -> EngineResult:
        ...
        command = [*args, *paths]
        # execute subprocess here (see ratchetr.engines.builtin.mypy for a template)
...
Use `ratchetr engines list` to inspect which engines were discovered (and where they came from), and `ratchetr cache clear`
to remove `.ratchetr_cache/` when you need to rebuild fingerprints from scratch.

### Engine Author Guide

Engines are lightweight adapters that translate `ratchetr` options into tool invocations and parse results.
...
## Configuration (ratchetr.toml)

Place a `ratchetr.toml` in the project root or pass `--config` when running the CLI. Example:
...
The `config_version` field is required and validated; ratchetr currently ships schema version `0`. All CLI flags
...
plugins or type stubs (for example, Django or DRF shims) may leave stale results behind. Clear `.ratchetr_cache/`
