...
- `typewiz query overview --manifest ...`: severity totals, with optional category and run summaries.
  - Flags: `--include-categories`, `--include-runs`, `--format json|table` (default: `json`).
- `typewiz query hotspots --manifest ...`: top files or folders by diagnostic volume.
  - Flags: `--kind files|folders` (default: `files`), `--limit N`, `--format json|table`.
- `typewiz query readiness --manifest ...`: readiness buckets for follow-up action.
...
- `typewiz query runs --manifest ...`: raw run metadata filtered by tool or mode.
  - Flags: `--tool NAME` (repeatable), `--mode current|full` (repeatable), `--limit N`, `--format json|table`.
- `typewiz query engines --manifest ...`: engine configuration applied to each run.
  - Flags: `--limit N`, `--format json|table`.
- `typewiz query rules --manifest ...`: most frequent rule identifiers.
  - Flags: `--limit N`, `--include-paths`, `--format json|table`.

Use `typewiz help query` for the latest flag defaults. For manifest field details see `docs/typewiz.md`, and for
